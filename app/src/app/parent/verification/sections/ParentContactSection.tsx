...
function normalisePhone(val: string): string {
  ...
  // If user entered "4XXXXXXXX" (without leading 0), prepend it
...
// Also accept 4XXXXXXXX (9 digits without leading 0) — normalized above
...
export function ParentContactSection({ verification, locked, onSaved }: ParentContactSectionProps) {
  ...
  function handlePhoneChange(val: string) {
    ...
    if (normalised && !AU_MOBILE_REGEX.test(normalised)) {
      setPhoneError("Enter an Australian mobile number (e.g. 0412 345 678)");
    } else {
      setPhoneError(null);
    }
  }
