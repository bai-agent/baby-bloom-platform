...
function PositionAccordion({ position }: { position: NonNullable<UpcomingIntro["position"]> }) {
        ...
        <div className="px-3 pb-3 space-y-3 border-t border-slate-200 pt-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {position.qualificationRequirement && (
                <span className="text-xs bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full">{position.qualificationRequirement}</span>
              )}
              {position.certificateRequirements?.map((cert) => (
                <span key={cert} className="text-xs bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full">{cert}</span>
              ))}
              {position.driversLicenseRequired && (
                <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">Drivers License</span>
              )}
              {position.carRequired && (
                <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">Own Car</span>
              )}
              {position.vaccinationRequired && (
                <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Vaccinated</span>
              )}
              {position.nonSmokerRequired && (
                <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Non-Smoker</span>
              )}
              {position.comfortableWithPetsRequired && (
                <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">Pet-Friendly</span>
              )}
              {position.languagePreference && (
                <span className="text-xs bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full">{position.languagePreference}</span>
              )}
            </div>
          </div>
          {position.otherRequirements && (
            <div>
              <p className="text-xs text-slate-400">Other Requirements</p>
              <p className="text-xs text-slate-700">{position.otherRequirements}</p>
            </div>
          )}
        </div>
      )}
